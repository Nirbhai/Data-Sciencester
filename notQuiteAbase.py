...
class Table:
    ...
    def select(self,
        ...
        # create a new table to return the results
    ...
    def where(self,
              predicate: WhereClause = lambda row: True) -> 'Table':
        """Return only the rows that satisfy the supplied predicate"""
        where_table = Table(self.columns, self.types)
        for row in self.rows:
            if predicate(row):
                new_row = [ row[column] for column in self.columns]
                where_table.insert(new_row)
        return where_table
...
assert len(users.select()) == 11

assert len(users.limit(2)) == 2
